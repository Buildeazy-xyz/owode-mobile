...
export default function ReceiptScreen({ route, navigation }: any) {
  ...
  const handleShare = async () => {
    await Share.share({
      message: `OWODE Alajo Receipt\n\nType: ${transaction.type}\nAmount: ₦${transaction.amount?.toLocaleString()}\nDescription: ${transaction.description}\nReference: ${transaction.reference}\nDate: ${new Date(transaction.createdAt).toLocaleString()}\n\nPowered by OWODE Digital Services`
    })
  }
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.receiptCard}>
        ...
        <View style={styles.poweredBy}>
          <Text style={styles.poweredByText}>🔒 Secured by OWODE Digital Services</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  receiptTitle: { fontSize: 18, fontWeight: 'bold', color: '#0d47a1', marginBottom: 20, textAlign: 'center' },
  ...
  poweredBy: { marginTop: 20, alignItems: 'center' },
  poweredByText: { fontSize: 12, color: '#888' },
